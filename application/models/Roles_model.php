...
class Roles_model extends CRM_Model
{
    ...
    public function add($data)
    {
        ...
        $this->db->where('code_role',$data['code_role']);
        $kiemtra=$this->db->get('tblroles')->row();
        if(!isset($kiemtra)) {
    ...
    public function update_type_role($id,$data)
    {
        $this->db->where('roleid',$id);
        $affectedRows= $this->db->update('tblroles',$data);
        if ($affectedRows > 0) {
            return true;
        }
        return false;
