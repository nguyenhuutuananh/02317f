...
class Staff_model extends CRM_Model
{
    ...
    public function get_insurrance($id)
    {
        $this->db->where('insurrance_staff_id',$id);
        return $this->db->get('tblstaff_insurrance')->row();
    }
    public function get_contract($id)
    {
        $this->db->where('contract_staff_id',$id);
        return $this->db->get('tblstaff_contract')->result_array();
    }
    public function get_information($id)
    {
        $this->db->where('staff_id',$id);
        return $this->db->get('tblstaff_information')->result_array();
    }
    public function get_comment($id)
    {
        $this->db->where('staffid_comment',$id);
        return $this->db->get('tblstaff_comment')->row();
    }
    ...
    public function get_code_staff($id,$idstaff="")
    {
        ...
        $requers=$this->db->get('tblroles')->row();
        if(isset($requers))
        {
            if($idstaff!="")
            {
                $this->db->where('staffid',$idstaff);
                $requ=$this->db->get('tblstaff')->row();
                if($requ->role==$id)
                {
                    return $requ->code_staff;
                }
                else
                {
                    $this->db->select("count(tblstaff.role) as count_staffid");
                    $this->db->join('tblroles','tblroles.roleid=tblstaff.role and tblstaff.role='.$id);
                    $requers_staff=$this->db->get('tblstaff')->row();
                    return $requers->code_role.($requers_staff->count_staffid+1);
                }
            }
            else
            {
    ...
    public function add($data)
    {
        ...
        if (isset($data['code_contract'])) {
            $data_contract['name_contract']=$data['name_contract'];
            $data_contract['code_contract']=$data['code_contract'];
            $data_contract['date_to']=$data['staff_contract_date'];
            $data_contract['to_date']=$data['staff_contract_to_date'];

            unset($data['name_contract']);
            unset($data['code_contract']);
            unset($data['staff_contract_date']);
            unset($data['staff_contract_to_date']);
        }
        if (isset($data['staff_information_date'])) {
            $data_information['date']=$data['staff_information_date'];
            $data_information['to_date']=$data['staff_information_to_date'];
            $data_information['address_job']=$data['staff_information_address_job'];
            $data_information['location_work']=$data['staff_information_location_work'];

            unset($data['staff_information_date']);
            unset($data['staff_information_to_date']);
            unset($data['staff_information_address_job']);
            unset($data['staff_information_location_work']);
        }

        $data_insurrance['code_xh']=$data['code_xh'];
        $data_insurrance['code_yt']=$data['code_yt'];
        $data_insurrance['date_gave']=$data['date_gave'];
        $data_insurrance['level']=$data['level'];
        $data_insurrance['mts']=$data['mts'];
        $data_insurrance['date_mts']=$data['date_mts'];
        $data_insurrance['people_off_1']=$data['people_off_1'];
        $data_insurrance['relation_1']=$data['relation_1'];
        $data_insurrance['people_off_2']=$data['people_off_2'];
        $data_insurrance['relation_2']=$data['relation_2'];
        $data_insurrance['people_off_3']=$data['people_off_3'];
        $data_insurrance['relation_3']=$data['relation_3'];
        unset($data['code_xh']);
        unset($data['code_yt']);
        unset($data['date_gave']);
        unset($data['level']);
        unset($data['mts']);
        unset($data['date_mts']);
        unset($data['people_off_1']);
        unset($data['relation_1']);
        unset($data['relation_2']);
        unset($data['people_off_2']);
        unset($data['relation_3']);
        unset($data['people_off_3']);
        if (isset($data['comment'])) {
            $data_comment['comment'] =$data['comment'];
            unset($data['comment']);
        }
        if (isset($data['comment_admin'])) {
            $data_comment['comment_admin'] =$data['comment_admin'];
        ...
        if ($staffid) {
            ...
            if (isset($data_contract)) {
                for ($i=0;$i<count($data_contract['name_contract']);$i++) {
                    $this->db->insert('tblstaff_contract', array(
                        'contract_staff_id' => $staffid,
                        'name_contract' => $data_contract['name_contract'][$i],
                        'code_contract' => $data_contract['code_contract'][$i],
                        'date_to' => $data_contract['date_to'][$i],
                        'to_date' => $data_contract['to_date'][$i]
                    ));
                }
            }
            if (isset($data_information)) {
                for ($i=0;$i<count($data_information['date']);$i++) {
                    $this->db->insert('tblstaff_information', array(
                        'staff_id' => $staffid,
                        'date' => $data_information['date'][$i],
                        'to_date' => $data_information['to_date'][$i],
                        'address_job' => $data_information['address_job'][$i],
                        'location_work' => $data_information['location_work'][$i]
                    ));
                }
            }
            if (isset($data_insurrance)) {
                $data_insurrance['insurrance_staff_id']=$staffid;
                $this->db->insert('tblstaff_insurrance',$data_insurrance);
            }

            if (isset($data_comment)) {
                $data_comment['staffid_comment']=$staffid;
                $this->db->insert('tblstaff_insurrance',$data_comment);
            }
    ...
    public function update($data, $id)
    {
        ...
        $this->db->where('staff_id',$id);
        $this->db->delete('tblstaff_information');

        $this->db->where('insurrance_staff_id',$id);
        $kt_insurrance=$this->db->get('tblstaff_insurrance')->row();

        if($kt_insurrance)
        {
            $this->db->where('insurrance_staff_id',$id);
            $this->db->update('tblstaff_insurrance',$data_insurrance);
        }
        else{
            $data_insurrance['insurrance_staff_id']=$id;
            $this->db->insert('tblstaff_insurrance',$data_insurrance);
        }

        $this->db->where('staffid_comment',$id);
        $su_comment=$this->db->get('tblstaff_comment')->row();
        if(isset($su_comment)){
            $this->db->where('staffid_comment',$id);
            $this->db->update('tblstaff_comment',$data_comment);
        }
        else
        {
            $data_comment['staffid_comment']=$id;
            $this->db->insert('tblstaff_comment',$data_comment);
        }

        $this->db->where('contract_staff_id',$id);
        $this->db->delete('tblstaff_contract');


        if (isset($data_contract)) {
            for ($i=0;$i<count($data_contract['name_contract']);$i++) {
                $this->db->insert('tblstaff_contract', array(
                    'contract_staff_id' => $id,
                    'name_contract' => $data_contract['name_contract'][$i],
                    'code_contract' => $data_contract['code_contract'][$i],
                    'date_to' => $data_contract['date_to'][$i],
                    'to_date' => $data_contract['to_date'][$i]
