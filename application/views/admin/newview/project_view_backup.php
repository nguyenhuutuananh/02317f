                             ...
                             <div class="clearfix"></div>
                                    ...
                                    <ul class="nav nav-tabs">
                                         ...
                                         <?php foreach($exigency as $rom=> $value){?>
                                            <?php if($rom==0){?>
                                                <li><a data-toggle="tab" href="#tab-<?=$value['id']?>"><?=$value['name']?></a></li>
                                            <?php } else {?>
                                                <li><a data-toggle="tab" href="#tab-<?=$value['id']?>"><?=$value['name']?></a></li>
                                            <?php }?>
                                         <?php }?>
                                                        ...
                                                        <thead>
                                                            <tr>
                                                                ...
                                                                <th class="bold">
                                                                    ...
                                                                    <?php echo render_select('furniture', $furniture,array('name','name'),'','',array()); ?>
                                                                </th>
                                                                ...
                                                                <th class="bold"><p class="text-center">Thời gian hết HĐ</p> <input class="form-control input-sm" id="deadline" placeholder="Thời gian hết HĐ"></th>
    ...
    <div id="content-script">
        <script>
            $( document ).ready(function() {
                $('.var_status').tooltip();
                $('#code,#province_name,#district_name,' +
                    '#type_pn,#door_direction_name,#fullname,#numberphone,' +
                    '#email,#furniture,#convenient,#characteristics,' +
                    '#date_update,#exigency_name,#price,#cost,#status,' +
                    '#detail_price,#deadline,#code_tax,#staff_id,#bonus,#note,#tag<?php if(count($render_colum)>0){foreach($render_colum as $rom){echo ',#'.$rom['id_input'].'_'.$rom['id_field'];}}?>').on('keyup', function () {


                    var list_input=['code','province_name','district_name','type_pn','door_direction_name',
                     'fullname','numberphone','email','furniture','convenient','characteristics','date_update',
                    'exigency_name','price','cost','status','detail_price','deadline','code_tax','staff_id','bonus','note','tag'<?php if(count($render_colum)>0){foreach($render_colum as $rom){echo ',"'.$rom['id_input'].'_'.$rom['id_field'].'"';}}?>];


                    var sum=0;
                    var row=0;
                    var project_tr = $('.tab-pane.active tbody tr');
                    $.each($(project_tr), function (i_tr, v_tr) {
                        var kiemtra=0;
                        $.each($(list_input), function (i, v) {
                            search_project = $('.tab-pane.active #' + v).val();
                            var _title=$('.tab-pane.active tr .'+v);
                            var inter=$(_title)[i_tr].title;
                            if(search(inter,search_project)!=true)
                            {
                                kiemtra=1;
                            }
                        })
                        if(kiemtra==1)
                        {
                            $(project_tr[i_tr]).hide();
                        }
                        else
                        {
                            $(project_tr[i_tr]).show();
                        }
                    })
                });
                ...
                $('#province_name,#district_name,' +
                    ...
                    $.each($(project_tr), function (i_tr, v_tr) {
                        ...
                        $.each($(list_input), function (i, v) {
                            search_project = $('.tab-pane.active #' + v).val();
                            var title=$('.tab-pane.active tr .'+v);
                            var inter=$(title)[i_tr].title;
                            if(search(inter,search_project)!=true)
                            {
                                kiemtra=1;
                            }
                ...
                function search(to, val) {
                    var res_to = to.toLowerCase();
                    var res_val = val.toLowerCase();
                    var i = 0;
                    if (res_to.search(res_val) != -1) {
                        i = 1;
                    }
                    if (i != 0) {
                        return true;
