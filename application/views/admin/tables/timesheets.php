...
foreach ($rResult as $aRow) {
    ...
    for ($i = 0; $i < count($aColumns); $i++) {
        ...
        if ($i == 0) {
            ...
            $_data = '<a href="'.admin_url('tasks/index/'.$aRow['task_id']).'" class="mtop5 inline-block" onclick="init_task_modal(' . $aRow['task_id'] . '); return false;">' .$aRow['name'] . '</a>';
            ...
            $_data .= '<span class="hidden"> - </span><span class="inline-block pull-right mtop5 mright5 label label-'.get_status_label($aRow['status']).'" task-status-table="'.$aRow['status'].'">' . format_task_status($aRow['status'],false,true).'</span>';
